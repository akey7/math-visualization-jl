...
function curve(; xs::Vector{Float64}, x_at_bottom::Float64, w::Float64, s::Float64, guess_lower::Float64, guess_upper::Float64)
    ys = zeros(Float64, length(xs))
    for (i, x) ∈ enumerate(xs)
        h = tension(x = x_at_bottom, w = w, s = s, guess_lower = guess_lower, guess_upper = guess_upper)
        if length(h) < 1
...
xs = collect(range(start = -10.0, stop = 15.0, length = 100))
ys = curve(xs = xs, x_at_bottom = 15.0, w = 5.0, s = 25.0, guess_lower = 30.0, guess_upper = 60.0)
plot(xs, ys)
```
